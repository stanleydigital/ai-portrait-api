...
// KV caching removed due to Vercel KV deprecation - poll.ts handles Cloudinary upload as fallback
    ...
    if (status === "succeeded" && output) {
      ...
      if (replicateUrl) {
        try {
          ...
          console.log("✅ Webhook: Uploaded to Cloudinary:", id, cdn_url);
        } catch (err) {
          console.error("❌ Webhook: Cloudinary upload failed:", err);
          // Don't fail the webhook - Replicate expects 200
          // poll.ts will handle the upload on next poll
        }
      }
    }

    if (status === "failed" || status === "canceled") {
      console.warn("⚠️ Webhook: Generation failed/canceled:", id, body.error);
    }
