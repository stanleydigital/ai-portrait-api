...
// ============================================
// API ENDPOINT: Store Order Data from Shopify
// ============================================
// This receives Shopify "Order Created" webhooks
// and stores portrait URLs in Vercel KV for later retrieval
...
// Verify Shopify webhook signature (CRITICAL for security)
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // Extract portrait data from line items
    const portraits: any[] = [];
    ...
    for (const item of order.line_items) {
      ...
      // Check if this line item has portrait data
    ...
    if (portraits.length === 0) {
      console.log("ℹ️ No portraits found in order", order.order_number);
    ...
    // Store for 90 days (plenty of time for customer to download)
    ...
    // Always return 200 so Shopify doesn't retry on temporary errors
    return new Response(JSON.stringify({ 
